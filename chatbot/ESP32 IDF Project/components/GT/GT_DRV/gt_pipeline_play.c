...
void gt_pipeline_init(gt_pipeline_st* gt_pipeline_obj)
{
    ...
    esp_audio_vol_set(gt_pipeline_obj->player, 80);
    AUDIO_MEM_SHOW(TAG);
}
#elif 0
...
void gt_pipeline_init(gt_pipeline_st* gt_pipeline_obj)
{
    ...
    ESP_LOGI(TAG, "[ 4 ] Set up  event listener");
    audio_event_iface_cfg_t evt_cfg = AUDIO_EVENT_IFACE_DEFAULT_CFG();
    evt_cfg.external_queue_size = 100;//32;
    evt_cfg.queue_set_size = 32;//12;
    gt_pipeline_obj->evt = audio_event_iface_init(&evt_cfg);
    // audio_event_iface_handle_t evt = audio_event_iface_init(&evt_cfg);
...
void gt_pipeline_init(gt_pipeline_st* gt_pipeline_obj)
{
   ...
   // audio_element_set_input_ringbuf(gt_pipeline_obj->i2s_stream_writer, gt_pipeline_obj->ringbuf);
    ...
    // ESP_LOGI(TAG, "[4.2] Listening event from peripherals");
    // audio_event_iface_set_listener(esp_periph_set_get_event_iface(set), gt_pipeline_obj->evt);
...
int gt_audio_pipeline_stop(gt_pipeline_st* gt_pipeline_obj)
{
    ...
    //rb_reset(gt_pipeline_obj->ringbuf);
    audio_pipeline_reset_ringbuffer(gt_pipeline_obj->pipeline);
#else
    // 丢弃所有未处理的事件
    audio_event_iface_discard(gt_pipeline_obj->evt);
...
{
    esp_audio_vol_set(gt_pipeline_obj->player, vol_value);
}